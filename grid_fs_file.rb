class GridFsFile
  ...
  def contents
    ...
    stream = nil
    buffer=""
    fs_bucket = self.class.mongo_client.database.fs
    stream = fs_bucket.open_download_stream( BSON::ObjectId.from_string(@id))
    if stream
      stream.each {|data|  buffer << data}
    end
    return buffer
  end
  ...
  def save
    ...
    if @contents
      ...
      #@id
      fs_bucket = self.class.mongo_client.database.fs
      id = fs_bucket.upload_from_stream(@filename, @contents, description)
      Rails.logger.debug {"saved gridfs file #{id}"}
      @id = id.to_s
    end
  end
